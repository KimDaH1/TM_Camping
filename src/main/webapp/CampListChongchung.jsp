...
if (request.getParameter("locations") == null) {
	...
	locationValue = request.getParameter("locations");
}
...
if (locationValue != null) {
	if (request.getMethod().equalsIgnoreCase("GET")) {
		locationValue = URLDecoder.decode(locationValue, "UTF-8");
	}
} else {//검색어가 없을 경우		
	locationValue = "";
}
...
if (!searchValue.equals("")) {
	...
	param += "&locations=" + locationValue;
}

//페이징 처리
String listUrl = "CampListChongchung.jsp?locations=" + locationValue;
...
function sendIt(){
	...
	f.action = "<%=cp%>/CampListChongchung.jsp";
	f.submit();
}
function clickTest() {
	...
	for (i = 1; i < rowList.length; i++) {//thead부분 제외.
		...
		row.onclick = function() {
			return function() {
				...
				var link = 'http://localhost:8080/ThreeMenCamping/ChongchungDetail.jsp?data='
						+ str;
...
window.onload = function() {
	document.querySelectorAll(".selectField")[1].value = "<%=locationValue%>";
}
</script>
			...
			<div id="leftHeader">
				<form action="" name="searchForm" method="post">
					<select name="searchKey" class="selectField">
						...
						class="textField" /> <input type="button" value="검  색"
						class="btn2" onclick="sendIt()"> <select name="locations"
						class="selectField">
